...
-- fresh project. Safe to re-run (uses IF NOT EXISTS / OR REPLACE / DROP ... IF
-- EXISTS guards throughout).
-- Source: docs/engineering/engineering-doc.md §7 (Database Design and Schema)
--
-- Also includes rate_limit_events (docs/security/security-plan.md §3),
-- backing lib/security/rateLimiter.ts — a DB-backed sliding window, correct
-- across multiple serverless instances unlike an in-memory counter.
-- ============================================================================
...
-- key_terms: extraction results (standard + custom terms share this table).
...
-- chat_messages: persisted chat turns. context_source records which
...
--    (docs/engineering/engineering-doc.md §8: trigger a prompt review if
...
-- (createAdminClient()) only, by design. RLS enabled with zero policies
-- means the table is fully inaccessible to anon/authenticated roles.
...
-- tables (including rate_limit_events) + 1 view, Storage shows the
-- "contracts" bucket, and Authentication > Policies shows RLS enabled on
-- every table — with zero policies on rate_limit_events by design.
-- ============================================================================
